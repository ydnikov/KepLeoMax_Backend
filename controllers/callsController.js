...
export const getOffer = async (req, res) => {
    ...
    if (!call || call.answerer_id !== userId) {
        return res.sendStatus(403);
    }

    const cachedOffer = offersCache[callId];
    if (!cachedOffer) {
        return res.sendStatus(404);
    }

    return res.status(200).json({ data: { type: cachedOffer.type, sdp: cachedOffer.sdp } });
}
...
export const newCall = async (req, res) => {
    ...
    const newCall = await callsModel.insertNewCall(userId, fcmToken, otherUserId);
...
export const acceptCall = async (req, res) => {
    ...
    await callsModel.setStartTime(callId, fcmToken);
    offersCache.delete(callId);
